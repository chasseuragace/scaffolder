/// Details page for LoyaltyCard.
import React from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { useLoyaltyCardById, useLoyaltyCardMutations } from '../hooks/loyalty-card.hooks';
...
export function LoyaltyCardDetailsPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data, isLoading, error, refetch } = useLoyaltyCardById(id ?? '');
  ...
  const { update, delete: deleteMutation } = useLoyaltyCardMutations({ onSuccess: setSuccessMessage });
  ...
  const handleDeleteConfirm = () => {
    deleteMutation.mutate(id ?? '', {
      onSuccess: () => navigate('/loyalty-card'),
    });
  ...
  if (!data) {
    return <div className="p-8 text-center">LoyaltyCard not found</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="rounded border p-6">
        <h1 className="mb-4 text-2xl font-bold">{data.name ?? 'Unnamed LoyaltyCard'}</h1>
