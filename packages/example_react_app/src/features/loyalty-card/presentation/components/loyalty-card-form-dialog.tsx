/// Form dialog for adding/editing LoyaltyCard.
///
...
interface LoyaltyCardFormDialogProps {
  isOpen: boolean;
  entity?: LoyaltyCardEntity;
  onClose: () => void;
  onSave: (entity: LoyaltyCardEntity) => void;
}

export function LoyaltyCardFormDialog({ isOpen, entity, onClose, onSave }: LoyaltyCardFormDialogProps) {
  const [formData, setFormData] = useState<Omit<LoyaltyCardEntity, 'id'> & { id: string }>({
    id: '',
    name: '',
    description: '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) onSave(formData as LoyaltyCardEntity);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg">
        <h2 className="mb-4 text-xl font-bold">
          {entity ? 'Edit LoyaltyCard' : 'Add LoyaltyCard'}
        </h2>
