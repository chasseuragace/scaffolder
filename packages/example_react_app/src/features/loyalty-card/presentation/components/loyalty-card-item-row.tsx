/// Item row component for LoyaltyCard list.
import type { LoyaltyCardEntity } from '../../domain/entities/loyalty-card.entity';

interface LoyaltyCardItemRowProps {
  item: LoyaltyCardEntity;
  onView: () => void;
  onDelete: () => void;
}

export function LoyaltyCardItemRow({ item, onView, onDelete }: LoyaltyCardItemRowProps) {
  return (
    <div className="flex items-center justify-between rounded border p-4 hover:bg-gray-50">
      <div className="flex-1">
        <h3 className="font-semibold">{item.name || 'Unnamed LoyaltyCard'}</h3>
        {item.description && (
