...
import { useCustomerListInfinite, useCustomerMutations } from '../hooks/customer.hooks';
...
import { CustomerItemRow } from '../components/customer-item-row';
...
export function CustomerListPage() {
  ...
  const { add, delete: deleteMutation } = useCustomerMutations({ onSuccess: setSuccessMessage });

  const { data: pages, isLoading, error, fetchNextPage, hasNextPage, isFetchingNextPage } = useCustomerListInfinite();
  ...
  if (!data || data.length === 0) {
    return <EmptyView message="No Customers yet" />;
  }
      ...
      <div className="space-y-2">
        {data.map((item) => (
          <CustomerItemRow
            key={item.id}
            item={item}
            onView={() => navigate(`/customer/${item.id}`)}
