/// Search bar component for Order.
///
...
import { useOrderSearch } from '../hooks/order.hooks';
import type { OrderEntity } from '../../domain/entities/order.entity';

interface OrderSearchBarProps {
  onSelect: (entity: OrderEntity) => void;
}

const DEBOUNCE_MS = 300;

export function OrderSearchBar({ onSelect }: OrderSearchBarProps) {
  const [query, setQuery] = useState('');
  const [debouncedQuery, setDebouncedQuery] = useState('');
  const [isOpen, setIsOpen] = useState(false);
  const timeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const { data, isLoading } = useOrderSearch(debouncedQuery);
  ...
  const handleSelect = (entity: OrderEntity) => {
    onSelect(entity);
    setQuery('');
    setDebouncedQuery('');
    setIsOpen(false);
