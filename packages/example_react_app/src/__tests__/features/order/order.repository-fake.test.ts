/// Tests for OrderRepositoryFake.
import { describe, it, expect, beforeEach } from 'vitest';
import { OrderRepositoryFake } from '../../../features/order/data/repositories/order.repository-fake';
import type { OrderEntity } from '../../../features/order/domain/entities/order.entity';

describe('OrderRepositoryFake', () => {
  let repository: OrderRepositoryFake;

  beforeEach(() => {
    repository = new OrderRepositoryFake();
  });
  ...
  it('should add an entity', async () => {
    ...
    if ('right' in result) {
      ...
      expect(result.right.name).toBe('Test Order');
    } else {
      throw new Error('Expected right');
    }
  });
  ...
  it('should get all entities', async () => {
    const entity1: OrderEntity = { id: '1', name: 'Test 1' };
    const entity2: OrderEntity = { id: '2', name: 'Test 2' };
  ...
  it('should get entity by id', async () => {
    const entity: OrderEntity = { id: '1', name: 'Test Order' };
  ...
  it('should update an entity', async () => {
    const entity: OrderEntity = { id: '1', name: 'Original' };
    await repository.add(entity);

    const updated: OrderEntity = { id: '1', name: 'Updated' };
  ...
  it('should search entities by name', async () => {
    const entity1: OrderEntity = { id: '1', name: 'Apple' };
    const entity2: OrderEntity = { id: '2', name: 'Banana' };
    const entity3: OrderEntity = { id: '3', name: 'Apricot' };
    ...
    if ('right' in result) {
      expect(result.right.length).toBe(2);
      expect(result.right.map((e: OrderEntity) => e.name)).toEqual(['Apple', 'Apricot']);
    } else {
      throw new Error('Expected right');
    }
  });
