...
class NotificationDetailsPage extends ConsumerWidget {
  const NotificationDetailsPage({super.key, required this.id});

  final String id;

  static Route<void> route(String id) => MaterialPageRoute(
        builder: (_) => NotificationDetailsPage(id: id),
      );

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(notificationByIdProvider(id));
    ...
    return Scaffold(
        ...
        actions: [
          async.maybeWhen(
            data: (entity) => IconButton(
              ...
              tooltip: 'Edit',
              onPressed: () =>
                  NotificationFormDialog.show(context, existing: entity),
            ),
      ...
      body: async.when(
        ...
        error: (e, _) => ErrorView(
          message: e.toString(),
          onRetry: () => ref.invalidate(notificationByIdProvider(id)),
        ),
