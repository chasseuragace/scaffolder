...
class NotificationItemTile extends ConsumerWidget {
  ...
  Future<void> _handleDelete(BuildContext context, WidgetRef ref) async {
    ...
    if (!ok) return;
    await ref.read(notificationListProvider.notifier).remove(item.id);
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Card(
      ...
      child: ListTile(
        onTap: () => Navigator.of(context).push(NotificationDetailsPage.route(item.id)),
          ...
          onSelected: (action) async {
            switch (action) {
              case _TileAction.edit:
                await NotificationFormDialog.show(context, existing: item);
