...
class _McpClient {
  ...
  static Future<_McpClient> start() async {
    ...
    //   <project>/mcp/test/integration_test.dart  →  <project>
    final scriptUri = Uri.parse('${Directory.current.path}/');
    final projectRoot = scriptUri.toFilePath().replaceAll(RegExp(r'/mcp/?$'), '');

    final proc = await Process.start(
      'dart',
      ['run', 'bin/main.dart'],
      workingDirectory: '$projectRoot/mcp',
      environment: {
        // Ensure the server auto-detects the embedded generator root,
        // not whatever the test runner's cwd is.
        'GENERATOR_ROOT': projectRoot,
